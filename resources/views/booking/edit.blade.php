...
<label> <strong> Booking date: </strong></label> 
    ...
    <select id="hour" name="hour" value="{{ $booking->booking_time}}" class="@error('booking_date') {{'is-invalid'}} @enderror">
    <?php for($i=12; $i<24; $i++):?>
            <option><?=$i?></option>
        <?php endfor; ?>
    </select>
    ...
    <select id="seats" name="seats" value="{{ $booking->seats_nbr }}" class="@error('booking_date') {{'is-invalid'}} @enderror">
        <?php for($i=1; $i<9; $i++):?>
            <option value="<?=$i?>"> <?=$i?> </option>
        <?php endfor; ?>
    </select>
